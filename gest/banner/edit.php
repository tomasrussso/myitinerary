<?php 
  ...
  CheckAuthBo();
    ...
    $event = $db->query('SELECT * FROM banner WHERE id = ?', $_GET['i'])->fetchArray();  
?>
<!DOCTYPE html>
<html lang="pt">
<head>
  ...
  <title>MyItinerary Gest | Editar Cidade do Banner - <?= $event['cityName'] ?></title>
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <form action="<?= BO_URL ?>/source/banner/edit.php" method="post">
      <div class="row">
        <div class="col-12">
        
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title"><?= $event['cityName'] ?></h3>
            ...
            <div class="card-body">
              <div class="form-group">
                ...
                <input type="text" name="name" id="inputName" value="<?= $event['cityName'] ?>" class="form-control" required>
              </div>
              <div class="form-group">
                <label for="inputName2">Descrição</label>
                <input type="text" name="description" id="inputName2" value="<?= $event['cityDescription'] ?>" class="form-control" required>
              </div>
              <div class="form-group">
                <label for="inputName3">Nome do Botão</label>
                <input type="text" name="button" id="inputName3" value="<?= $event['buttonCityName'] ?>" class="form-control">
                <p class="mt-1" style="color: #909090">Opcional</p>
              </div>
              <div class="form-group">
                <label for="inputName4">Caminho da Imagem</label>
                <input type="text" name="image" id="inputName4" value="<?= $event['imagePath'] ?>" class="form-control" required>
              </div>
              <div class="form-group">
                <label for="inputName5">Créditos</label>
                <input type="text" name="credits" id="inputName5" value="<?= $event['credits'] ?>" class="form-control" required>
              </div>
              <div class="form-group">
                <label for="inputName6">Estado</label>
                <input type="text" name="status" id="inputName6" value="<?= $event['status'] ?>" class="form-control" required>
              </div>
            </div>
            <!-- /.card-body -->
          </div>
